...
def attack(image, image_o, label, model, lossFunction):
    model.zero_grad()
    image = image.clone().detach().to("cuda:0")
    image.requires_grad = True

    loss = lossFunction(model(image)[1], label)
    loss.backward()

    image = image + 0.01 *image.grad
    image = torch.max(torch.min(image, image_o + 0.03), image_o - 0.03)
    return image
